...
class ThChSubmission(SubmissionPy):
    def run(self, s):
        ...
        for inp, (prev_inp, delta) in constraints.items():
            ...
            # To get the highest model number, we need prev_inp as big as possible
            ...
            #    So highest possible value for prev_inp is min(9 - delta, 9)
            #    And then inp = prev_inp + delta
            #             inp = min(9 - delta, 9) + delta
            #             inp = min(9, 9 + delta)
            ########
            values[prev_inp] = min(9 - delta, 9)
            values[inp] = min(9, 9 + delta)
