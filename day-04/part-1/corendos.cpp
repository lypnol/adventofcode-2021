...
void load_and_process_grid(Iterator *it, Context *context) {
    ...
    u8 column_index_of_win = grid_min(processing.column_index_of_win[0], processing.column_index_of_win[1], processing.column_index_of_win[2], processing.column_index_of_win[3], processing.column_index_of_win[4]);
    u8 grid_winning_index = std::min(line_index_of_win, column_index_of_win);
    if (grid_winning_index < context->drawing_win) {
        context->index_winning_grid = context->grid_count;
        context->drawing_win = std::min(context->drawing_win, grid_winning_index);
    }
    context->grid_count++;
}
