...
trait Set<T> {
    ...
    fn joker_user_and_contains(&self, node: T) -> bool;
...
trait GraphForDay {
    ...
    fn count_paths(&mut self, current: Self::Node, visited: Self::NodeSet) -> u32 {
        ...
        if self.is_end(current) || visited.joker_user_and_contains(current) {
            return 0;
        }
    ...
    fn count_paths_to_end(&mut self) -> u32 {
        self.predecessors(self.end()).fold(0, |acc, p| {
            acc + self.count_paths(p, Self::NodeSet::empty())
        })
    }
}
...
impl Set<u32> for NSet {
    /// The following assumes that `start` never enters the set.
    ...
    #[inline(always)]
    fn joker_user_and_contains(&self, node: u32) -> bool {
        let n_and_j = node | JOKER;
        self.contains(n_and_j)
    }
