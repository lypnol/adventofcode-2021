...
#define MAX_SIMULTANEOUS_PATHS 10000
...
unsigned long run(char* s) {
    ...
    paths[len].seen_small_twice = false;
    for (unsigned short idx = 0; idx < graph.n_caves; idx++) {
        paths[len].visited[idx] = false;
    }
    len++;
    while (len) {
        // pop
        ...
        for (unsigned short next=0; next < graph.n_caves; next++) {
            if (graph.adjency_matrix[paths[path_idx].current][next]) {
                ...
                if (next == END) {counter++; continue;}
                if (paths[path_idx].seen_small_twice && paths[path_idx].visited[next]) {continue;}
                // push
                ...
                paths[len].seen_small_twice = paths[path_idx].seen_small_twice || paths[path_idx].visited[next];
                len++;
            }
        }
        len--;
        if (path_idx < len) {
            ...
            paths[path_idx].seen_small_twice = paths[len].seen_small_twice;
            for (unsigned short idx = 0; idx < graph.n_caves; idx++) {
