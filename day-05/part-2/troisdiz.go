...
func getPoints(line *Line) []Point {
	...
	incrX, absX := signAndAbs(line.end.x - line.start.x)
	incrY, absY := signAndAbs(line.end.y - line.start.y)
	delta := max(absX, absY)

	for idx := 0; idx <= delta; idx++ {
		result = append(result, Point{ line.start.x + idx*incrX, line.start.y + idx*incrY})
	}
	return result
}
