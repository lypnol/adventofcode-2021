...
impl Line {
    fn from_str(s: &str) -> Option<Self> {
        ...
        let x1: Coord = splits.next().unwrap().parse().ok()?;
        let y1: Coord = splits.next().unwrap().parse().ok()?;
        let _ = splits.next();
        let x2: Coord = splits.next().unwrap().parse().ok()?;
        let y2: Coord = splits.next().unwrap().parse().ok()?;
        if x1 == x2 || y1 == y2 {
            Some(Self {
                x1: x1.min(x2),
                y1: y1.min(y2),
...
fn run(input: &str) -> usize {
    ...
    for l in input.lines() {
        if let Some(new_line) = Line::from_str(l) {
