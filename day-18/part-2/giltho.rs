...
impl JellyfishNumber {
    ...
    fn parse(input: &[u8]) -> Self {
        ...
        ret.add_from_input(input);
        ret
    }

    fn add_from_input(&mut self, input: &[u8]) {
...
fn run(input: &[u8]) -> u32 {
    let mut max_magn = 0;
    for (i, line1) in input.split(|b| *b == b'\n').enumerate() {
        let jf = JellyfishNumber::parse(line1);
        for (j, line2) in input.split(|b| *b == b'\n').enumerate() {
            if i != j {
                let mut jf = jf.clone();
                jf.add_from_input(line2);
                max_magn = std::cmp::max(max_magn, jf.magnitude())
            }
        }
    }
    max_magn
...
mod tests {
    ...
    fn parse(data: &str) -> JellyfishNumber {
        JellyfishNumber::parse(data.as_bytes())
    }
    ...
    fn parse_add() {
        ...
        jf.add_from_input(right.as_bytes());
