...
fn run(input: &str) -> usize {
    ...
    for i in 0..n - 1 {
        for j in i + 1..n {
            ...
            let fi = hw.parse_fish(lines[i]);
            let fj = hw.parse_fish(lines[j]);
            let mut hw2 = hw.clone();
            let a1 = hw.addition(fi, fj);
            hw.reduce(a1);
            max = max.max(hw.magnitude(a1));
            let a1 = hw2.addition(fj, fi);
            hw2.reduce(a1);
            max = max.max(hw2.magnitude(a1))
        }
    }
    max
}
...
[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]"),
    ...
    fn test_add_reduce() {
        ...
        hw.reduce(root);
        // assert_eq!(hw.to_string(root), "[[[[4,0],[5,4]],[[7,7],[6,0]]],[[8,[7,7]],[[7,9],[5,0]]]]");
        let new = hw.parse_fish("[[2,[[0,8],[3,4]]],[[[6,7],1],[7,[1,6]]]]");
        root = hw.addition(root, new);
        hw.reduce(root);
        // assert_eq!(hw.to_string(root), "[[[[6,7],[6,7]],[[7,7],[0,7]]],[[[8,7],[7,7]],[[8,8],[8,0]]]]");
