...
func NewSnailfishNumberFromAdd(sn1, sn2 SnailfishNumber) SnailfishNumber {
	sn := SnailfishNumber{Left: &sn1, Right: &sn2}
	sn.Reduce()
	return sn
}
...
func run(s string) int {
	...
	sn, _ := NewSnailfishNumberFromLine(lines[0], 0)
	for _, line := range lines[1:] {
		ssn, _ := NewSnailfishNumberFromLine(line, 0)
		sn = NewSnailfishNumberFromAdd(sn, ssn)
	}
